...
export default function Landing() {
  ...
  return (
    <div style={{
      ...
      {/* ── NAV ─────────────────────────────────────────────── */}
      <nav className="anim-down sticky top-0 z-50 flex items-center justify-between px-8 py-4"
        style={{ borderBottom: `3px solid ${t.border}`, background: t.bg2, transition: 'background 0.3s ease' }}>
      ...
      <section className="px-8 pt-20 pb-16 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          {/* Left col — text */}
          <div>
            <div className="anim-left inline-flex items-center gap-2 px-3 py-1 mb-6"
            ...
            <h1 className="anim-left delay-1 text-5xl lg:text-7xl font-black uppercase leading-none mb-6" style={{ color: t.text }}>
              INCIDENTS<br />
            ...
            <p className="anim-left delay-2 text-lg font-medium mb-8 max-w-md" style={{ color: t.muted }}>
            ...
            <div className="anim-left delay-3 flex gap-4 flex-wrap">
              <Link to="/register"
            ...
            <div className="anim-left delay-4 flex gap-3 mt-8 flex-wrap">
              {['No Credit Card', 'Free to Start', 'AI-Powered'].map(b => (
                <span key={b} className="flex items-center gap-1.5 text-xs font-bold uppercase" style={{ color: t.muted }}>
          ...
          <div className="anim-right relative">
            <div className="absolute -top-4 -right-4 w-16 h-16 z-10"
              style={{ background: '#FF2D78', border: `2px solid ${t.border}`, boxShadow: `3px 3px 0 ${t.border}` }} />
            <div className="absolute -bottom-4 -left-4 w-10 h-10 z-10"
              style={{ background: '#FF8C42', border: `2px solid ${t.border}`, boxShadow: `2px 2px 0 ${t.border}` }} />
            ...
            <div style={{ ...card(), position: 'relative', overflow: 'hidden' }}>
              <div className="flex items-center justify-between px-4 py-3 -mx-6 -mt-6 mb-5"
                ...
                <span className="text-xs font-black uppercase tracking-wider" style={{ color: '#FF8C42' }}>FixFlow Dashboard</span>
      ...
      {/* ── STATS BAR ────────────────────────────────────────── */}
      <section className="anim-up delay-3"
        style={{ borderTop: `3px solid ${t.border}`, borderBottom: `3px solid ${t.border}`, background: t.accentBg }}>
      ...
      <section id="features" className="px-8 py-20 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {FEATURES.map(({ icon: Icon, title, desc, accent }, i) => (
            <div key={title}
              className={`p-5 group transition-all ${i % 2 === 0 ? 'anim-left' : 'anim-right'} delay-${i + 1}`}
              style={card()}
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            {HOW.map(({ step, title, desc }, i) => (
              <div key={step} className={`relative p-6 anim-up delay-${i + 1}`} style={card()}>
      ...
      <section id="stats" className="px-8 py-20 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={tech.name} className={`p-4 text-center transition-all anim-up delay-${(i % 4) + 1}`}
              style={card()}
              onMouseEnter={e => { e.currentTarget.style.transform = 'translate(-2px,-2px)'; e.currentTarget.style.boxShadow = `6px 6px 0 ${t.border}`; }}
              onMouseLeave={e => { e.currentTarget.style.transform = 'none'; e.currentTarget.style.boxShadow = `4px 4px 0 ${t.border}`; }}>
      ...
      {/* ── CTA ──────────────────────────────────────────────── */}
      <section style={{ borderTop: `3px solid ${t.border}`, background: t.accentBg }} className="px-8 py-24">
        <div className="max-w-3xl mx-auto text-center anim-up">
      ...
      {/* ── FOOTER ──────────────────────────────────────────── */}
      <footer style={{ borderTop: `3px solid ${t.border}`, background: t.bg2 }} className="px-8 py-8">
        <div className="max-w-7xl mx-auto flex items-center justify-between flex-wrap gap-4">
          ...
          <div className="flex items-center gap-4">
            <Link to="/login" className="text-xs font-bold uppercase" style={{ color: t.muted }}>Login</Link>
            <Link to="/register" className="text-xs font-bold uppercase" style={{ color: t.muted }}>Register</Link>
            <button onClick={toggle}
              className="w-7 h-7 flex items-center justify-center"
              style={{ border: `2px solid ${t.border}`, background: t.bg3 }}>
              {dark ? <Sun size={12} color={t.text} /> : <Moon size={15} color={t.text} />}
            </button>
          </div>
        </div>
