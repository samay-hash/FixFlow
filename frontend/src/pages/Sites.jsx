...
export default function Sites() {
  ...
  const load = async () => {
    try {
      ...
      setSites(data.sites);
    } catch (err) { toast.error('Failed to load sites'); }
  ...
  const handleDelete = async (id, name) => {
    ...
    } catch (err) { toast.error('Failed to delete site'); }
  };
      ...
      <main className="flex-1 ml-64 p-8">
        <div className="page-header flex items-center justify-between">
          <div>
            ...
            <p className="page-subtitle">{sites.length} sites • {upSites} up {downSites > 0 && `• ${downSites} DOWN`}</p>
          </div>
          <div className="flex gap-3">
            <button onClick={load} className="btn-ghost btn-sm"><RefreshCw size={14} />Refresh</button>
            {isAdmin && <button onClick={() => setShowForm(!showForm)} className="btn-primary"><Plus size={16} />Add Site</button>}
          </div>
        </div>

        {/* Add Site Form */}
        {showForm && (
          <div className="card mb-6 animate-fade-in border-blue-500/30">
            <h3 className="font-semibold text-white mb-4">Add New Site to Monitor</h3>
            <form onSubmit={handleAdd} className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <select className="input" value={form.checkInterval} onChange={e => setForm({ ...form, checkInterval: Number(e.target.value) })}>
                  {[1, 2, 5, 10, 15, 30].map(v => <option key={v} value={v}>{v} min</option>)}
                </select>
              </div>
              <div className="md:col-span-3 flex gap-3">
                <button type="submit" disabled={adding} className="btn-primary">{adding ? 'Adding...' : 'Add Site'}</button>
        ...
        {loading ? (
          ...
          <div className="card text-center py-16">
            ...
            <p className="text-slate-400 text-sm mb-4">Add your first website to start monitoring</p>
            {isAdmin && <button onClick={() => setShowForm(true)} className="btn-primary mx-auto"><Plus size={16} />Add Site</button>}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {sites.map(site => (
              <div key={site._id} className={clsx('card-hover group relative', site.status === 'down' && 'border-red-500/40')}>
                {site.status === 'down' && <div className="absolute top-3 right-3"><span className="pulse-dot-red" /></div>}

                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2">
                    {site.status === 'down' ? <WifiOff size={18} className="text-red-400" /> : <Wifi size={18} className="text-emerald-400" />}
                    <h3 className="font-semibold text-white text-sm">{site.name}</h3>
                  </div>
                  <StatusBadge s={site.status} />
                </div>

                <a href={site.url} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-xs text-blue-400 hover:text-blue-300 mb-4 truncate">
                  <ExternalLink size={10} />{site.url}
                </a>

                <div className="grid grid-cols-3 gap-3 mb-4">
                  <div className="text-center p-2 bg-dark-800/60 rounded-lg">
                    <p className="text-sm font-bold text-white">{site.uptimePercent}%</p>
                    <p className="text-xs text-slate-500">Uptime</p>
                  </div>
                  <div className="text-center p-2 bg-dark-800/60 rounded-lg">
                    <p className="text-sm font-bold text-white">{site.responseTime || '—'}<span className="text-xs">ms</span></p>
                    <p className="text-xs text-slate-500">Response</p>
                  </div>
                  <div className="text-center p-2 bg-dark-800/60 rounded-lg">
                    <p className="text-sm font-bold text-white">{site.checkInterval}m</p>
                ...
                <div className="flex items-center justify-between">
                  ...
                  {user?.role === 'admin' && (
                    <button onClick={() => handleDelete(site._id, site.name)}
