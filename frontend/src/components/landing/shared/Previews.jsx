...
import { Mail, AlertTriangle, GitBranch, Sparkles } from 'lucide-react';
import { SparkLine, SmallBars } from './UIAtoms';

export function AlertLogos({ compact = false }) {
  const base = compact ? 'h-9 w-9 rounded-xl' : 'h-10 w-10 rounded-xl';
  return (
    <div className="flex items-center gap-2">
      <span className={`${base} flex items-center justify-center bg-[#4a154b] text-[11px] font-black text-white`}>SL</span>
      <span className={`${base} flex items-center justify-center border border-slate-200 bg-white text-slate-600`}><Mail size={18} /></span>
      <span className={`${base} flex items-center justify-center bg-emerald-50 text-[18px] font-black text-emerald-600`}>pd</span>
...
export function TrendPreview({ color = '#22c55e' }) {
  return (
  ...
  return (
    <div className="ml-auto w-52 rounded-2xl border border-slate-100 bg-white p-3 text-[10px] shadow-sm">
      <div className="mb-2 flex items-center gap-2">
        ...
        <GitBranch size={16} className="ml-auto text-slate-800" />
      </div>
