...
const normalizePreferences = (preferences: any): string[] => {
...
const generateToken = (id: any) =>
  jwt.sign({ id } as any, process.env.JWT_SECRET as string, { expiresIn: (process.env.JWT_EXPIRES_IN || '7d') as any });
...
export const getTeam = async (req: AuthRequest, res: Response) => {
  try {
    const [team, invites] = await Promise.all([
      User.find({ companyId: req.user?.companyId as any }).select('-password'),
      Invite.find({ companyId: req.user?.companyId as any }).sort({ createdAt: -1 }),
    ]);
...
export const getUnassignedUsers = async (req: AuthRequest, res: Response) => {
  try {
    const companyId = req.user?.companyId;

    // aggregation to find counts of active assignments per user
    const agg = await Incident.aggregate([
      { $match: { companyId: new mongoose.Types.ObjectId(companyId as any), status: { $ne: 'resolved' } } },
    ...
    const users = await User.find({ companyId: companyId as any }).select('-password');
...
export const inviteTeamMember = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const company = await Company.findById(req.user?.companyId as any);
...
export const acceptInvite = async (req: Request, res: Response) => {
  try {
    ...
    if (user && String(user.companyId) !== String((invite.companyId as any)._id)) {
    ...
    const company = await Company.findById((invite.companyId as any)._id);
    res.json({
      success: true,
      token: generateToken(user._id),
