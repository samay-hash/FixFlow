...
export const sendDownAlert = async (site: IWebsite, incident: IIncident) => {
  ...
  await sendSlackNotification(`🚨 *CRITICAL ALERT*: ${site.name} is DOWN!`, [
    {
      "type": "header",
      "text": { "type": "plain_text", "text": "🚨 Production Site Down", "emoji": true }
    },
    {
      "type": "section",
      "fields": [
    ...
    },
    {
      ...
      "elements": [
        {
          "type": "button",
          "text": { "type": "plain_text", "text": "View Incident & Respond", "emoji": true },
          "url": incidentUrl,
          "style": "danger"
        }
      ]
    ...
    html: `
      <div style="font-family: 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; background: #f9f9f9; border-radius: 8px; overflow: hidden; border: 1px solid #e0e0e0;">
        <div style="background: #FF2D55; padding: 24px 32px;">
          <h1 style="margin:0; color:white; font-size:22px; font-weight:800;">🚨 Production Site Down</h1>
          <p style="margin:6px 0 0; color:rgba(255,255,255,0.85); font-size:14px;">FixFlow Incident Alert System</p>
        </div>
        <div style="padding: 28px 32px; background: #ffffff;">
          ...
          <div style="margin-top:24px; padding:16px; background:#FFF5F5; border-left:4px solid #FF2D55; border-radius:4px;">
            <p style="margin:0; color:#c0392b; font-size:13px; font-weight:600;">⚡ FixFlow has automatically created an incident. Your team has been notified.</p>
          </div>
          <a href="${incidentUrl}" style="display:inline-block;margin-top:24px;padding:14px 28px;background:#0050FF;color:white;border-radius:6px;text-decoration:none;font-weight:700;font-size:14px;letter-spacing:0.5px;">View Incident & Respond →</a>
        </div>
        <div style="padding:16px 32px; background:#f5f5f5; text-align:center; border-top:1px solid #e0e0e0;">
          <p style="margin:0; color:#999; font-size:12px;">FixFlow — Smart Incident Monitoring & Response<br/>You're receiving this because you're an admin/engineer in this workspace.</p>
        </div>
      </div>
    `,
  });
...
export const sendInviteEmail = async ({ to, inviterName, companyName, inviteUrl, role, category }: InviteOptions) => {
  await sendEmail({
    to,
    ...
    html: `
      <div style="font-family: 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; background: #f9f9f9; border-radius: 8px; overflow: hidden; border: 1px solid #e0e0e0;">
        <div style="background: #0050FF; padding: 24px 32px;">
          <h1 style="margin:0; color:white; font-size:22px; font-weight:800;">You've Been Invited 🎉</h1>
          <p style="margin:6px 0 0; color:rgba(255,255,255,0.85); font-size:14px;">FixFlow — Smart Incident Monitoring</p>
        </div>
        <div style="padding: 28px 32px; background: #ffffff;">
          ...
          <div style="margin:20px 0; padding:16px; background:#F0F4FF; border-left:4px solid #0050FF; border-radius:4px;">
            <p style="margin:0; color:#333; font-size:13px;">FixFlow helps your team monitor uptime, respond to incidents faster, and auto-generate AI postmortems.</p>
          </div>
          <a href="${inviteUrl}" style="display:inline-block;padding:14px 28px;background:#0050FF;color:white;border-radius:6px;text-decoration:none;font-weight:700;font-size:14px;">Accept Invitation →</a>
          <p style="margin-top:16px; color:#999; font-size:12px;">This invite expires in 7 days. If you didn't expect this, you can safely ignore it.</p>
        </div>
        <div style="padding:16px 32px; background:#f5f5f5; text-align:center; border-top:1px solid #e0e0e0;">
...
export const sendAssignedIncidentEmail = async (incident: IIncident, assignees: any[]) => {
    ...
    try {
      ...
      <div style="font-family: sans-serif; max-width: 600px;">
        ...
        <div style="background: #1a1a2e; color: #e2e8f0; padding: 24px; border-radius: 0 0 8px 8px;">
          ...
          <p><strong>Site:</strong> ${(incident.siteId as any)?.name || '—'} ${(incident.siteId as any)?.url ? `(${(incident.siteId as any).url})` : ''}</p>
          <p><strong>Description:</strong></p>
          <p>${incident.description || 'No description provided.'}</p>
          <p style="margin-top:12px;">
            <a href="${process.env.FRONTEND_URL}/incidents/${incident._id}" style="display:inline-block;padding:10px 18px;background:#3b82f6;color:white;border-radius:6px;text-decoration:none;">View Incident</a>
          </p>
...
export const sendSlackNotification = async (text: string, blocks?: any[]) => {
