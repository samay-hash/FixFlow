...
import { IPostmortem } from '../models/Postmortem';
...
const buildPostmortemPrompt = (incident: IIncident): string => {
...
- Affected Site: ${(incident.siteId as any)?.url || 'Unknown'}
- Responders: ${incident.assignedTo?.length ? 'Responders assigned' : 'None assigned'}
...
export const generatePostmortem = async (incident: IIncident): Promise<PostmortemDraft> => {
  try {
    ...
    if (geminiKey) {
      ...
      } catch (e: any) { logger.warn(`Gemini postmortem failed: ${e.message} — trying Groq`); }
    }
    ...
    if (groqKey) {
      ...
      } catch (e: any) { logger.warn(`Groq postmortem failed: ${e.message} — using mock`); }
    }
...
export const scorePostmortem = async (postmortem: IPostmortem): Promise<{ score: number; feedback: string }> => {
  try {
...
export const generateSitrep = async (incident: IIncident, logs: any[] = []): Promise<string> => {
  const geminiKey = process.env.GEMINI_API_KEY;
  const groqKey   = process.env.GROQ_API_KEY;
  ...
  Duration: ${Math.round((Date.now() - (incident.createdAt as Date).getTime()) / 60000)} minutes
  ...
  try {
    ...
    if (geminiKey) {
      try {
        ...
        if (text) return text;
      } catch (e: any) { logger.warn(`Gemini SITREP failed: ${e.message} — trying Groq`); }
    }
    ...
    if (groqKey) {
      try {
        ...
        if (text) return text;
      } catch (e: any) { logger.warn(`Groq SITREP failed: ${e.message}`); }
    }
...
export const analyzeLogs = async (logs: any[]): Promise<LogAnalysisResult> => {
  const geminiKey = process.env.GEMINI_API_KEY;
  const groqKey   = process.env.GROQ_API_KEY;
  ...
  if (geminiKey) {
    ...
    } catch (e: any) { logger.warn(`Gemini log analysis failed: ${e.message} — trying Groq`); }
  }
  ...
  if (groqKey) {
    ...
    } catch (e: any) { logger.warn(`Groq log analysis failed: ${e.message} — using local scan`); }
  }

  // ── Smart local fallback (no AI needed) ──────────────────────────────
  logger.warn('No AI key available — using local log pattern scan');
  const fatals   = logs.filter(l => l.level === 'FATAL' || l.level === 'fatal');
  const errors   = logs.filter(l => l.level === 'ERROR' || l.level === 'error');
  const warnings = logs.filter(l => l.level === 'WARN' || l.level === 'warning');
...
const generateMockPostmortem = (incident: IIncident): PostmortemDraft => ({
  summary: `A ${incident.severity} severity incident was detected affecting ${(incident.siteId as any)?.name || 'the monitored service'}. The incident was ${incident.status} after ${incident.mttr ? Math.round(incident.mttr / 60) + ' minutes' : 'an unknown duration'}.`,
...
export const askGroqCopilot = async (incident: IIncident, logs: any[], userMessage: string, chatHistory: any[] = []): Promise<string> => {
  try {
